...
def admin():
    ...
    py_compile.compile("utils/Login.py", "utils/Login.pyc") #compiles Login file
...
def labGen():
    ...
    pwd.append(''.join(random.choices(string.ascii_uppercase + string.ascii_lowercase + string.digits, k=8)))
    ...
    for i in range(1,6):
        studentSolution[f"Q{i+1}A"] = f"{gang[i]}" #for autograding, record username
        studentSolution[f"Q{i+1}B"] = f"{pwd[i]}" #for autograding, record password
    for i in range(6, len(gang)):
        pwd.append(''.join(random.choices(string.ascii_uppercase + string.ascii_lowercase + string.digits, k=15)))
    ...
    for i in range(len(gang)):
        hash = hashlib.sha256()
        hash.update(bytes(gang[i]+pwd[i],'utf-8'))  # simple defenses against dictionary attack
        for j in range(90000): hash.update(hash.digest())  # prepend name and iterate
        hashed = hash.hexdigest()
    ...
    for i in range(total):
        q4.append(f"{random.choice(names + gang)},{''.join(random.choices(string.ascii_uppercase + string.ascii_lowercase + string.digits, k=8))}") #append a gang member with an incorrect password, or random name
