...
void *printer_main(void *arg) {
    // TODO
    printer_t *printTime = (printer_t *) arg;
    job_queue_t *jobQueue = printTime->jq;
    int complete = 0;
    ...
    while (!complete) {
        ...
        pthread_mutex_lock(&jobQueue->mutex);
        int job = q_fetch_job(jobQueue, printTime->id);
        pthread_mutex_unlock(&jobQueue->mutex);
        print_job(job);
...
int main(int argc, char *argv[]) {
     ...
     *  */
    pthread_t all_threads[num_printers];
    ...
    for (int i = 0; i < num_printers; i++) {
        ...
        printers[i].njobs = 0;
        pthread_create(&all_threads[i], NULL, printer_main, &printers[i]);
    }
    for (int i = 0; i < num_printers; i++) {
