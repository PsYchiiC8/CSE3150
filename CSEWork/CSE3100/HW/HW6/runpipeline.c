...
void start_program(Program *programs, int num_programs, int cur){
    ...
    if(childProcess == 0){
        if(0 < cur){
            if(0 < dup2(prog->fd_in, FD_STDIN)){
                die("dup2() failed");
            }
        }
        if(cur != num_programs - 1){
            if(dup2(prog->fd_out, FD_STDOUT) < 0){
                die("dup2() failed");
            }
        }

        for(int i = cur; i < num_programs; i++){
            if (0 < programs[i].fd_in){
                if(close(programs[i].fd_in) < 0) {
                    die("close() failed");
                }
            }
            if(0 < programs[i].fd_out){
                if(close(programs[i].fd_out) < 0) {
                    die("close() failed");
                }
            }
        }
        ...
        prog->pid = childProcess;

        if(0 < prog->fd_in && 0 < cur){
            if(close(prog->fd_in) < 0){
                die("close() failed");
            }
        }
        if(cur != num_programs - 1 && 0 < prog->fd_out){
            if(0 < close(prog->fd_out)){
                die("close() failed");
            }
        }
    }
...
int wait_on_program(Program *prog)
{
    ...
    // TODO
    if(waitpid(prog->pid, &exitStatus, 0) != prog->pid){
        return -1;
    }
...
void prepare_pipes(Program *programs, int num_programs)
{
    ...
    for(int i = 0; i < num_programs - 1; i++){
        ...
        programs[i + 1].fd_in = fd[0];
        programs[i].fd_out = fd[1];
    }
}
