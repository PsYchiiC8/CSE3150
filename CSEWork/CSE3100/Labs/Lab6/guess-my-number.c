...
void readpipe(int pipe[], int *result){
    if (read(pipe[PIPEFD_READ], result, sizeof(*result)) < 0){
        die("read() failed.");
    }
}

void readpipeChar(int pipe[], char *result){
    if (read(pipe[PIPEFD_READ], result, sizeof(*result)) < 0){
        die("read() failed.");
    }
}

void writepipe(int pipe[], int *result){
    if (write(pipe[PIPEFD_WRITE], result, sizeof(*result)) < 0){
        die("write() failed.");
    }
}

void writepipeChar(int pipe[], char *result){
    if (write(pipe[PIPEFD_WRITE], result, sizeof(*result)) < 0){
        die("write() failed.");
    }
}
...
void    child_main(int fdp[], int fdc[], int seed)
{
    ...
    int guess;
    int result;
    char resultMsg[MSG_BUF_SIZE];

    result = gmn_get_max();
    writepipe(fdc, &result);
    ...
    strcpy(resultMsg, gmn_get_message(&gmn));

    int i = 0;

    while (resultMsg[i] != '\0'){
        writepipeChar(fdc, &resultMsg[i]);
        i++;
    }
