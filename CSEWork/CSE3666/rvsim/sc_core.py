...
def RISCVCore(imem_data, dmem_data, rf, clock, reset, env):
    ...
    iMem = Rom(sig.instruction, sig.PC, imem_data)
    u_mainControl = MainControl(sig.opcode, sig.ALUOp, sig.ALUSrc, sig.Branch, sig.MemRead, sig.MemWrite, sig.MemtoReg, sig.RegWrite)
    u_RegFile = RegisterFile(sig.ReadData1, sig.ReadData2, sig.rs1, sig.rs2, sig.rd, sig.WriteData, sig.RegWrite, rf, clock, posedge=True)
    u_iGen = ImmGen(sig.immediate, sig.instruction)
    ...
    u_DM = Ram(sig.MemReadData, sig.ReadData2, sig.ALUResult, sig.MemRead, sig.MemWrite, dmem_data, clock)

    u_Mux2_0 = Mux2(sig.NextPC, sig.PC4, sig.BranchTarget, sig.PCSrc)
    u_Mux2_1 = Mux2(sig.ALUInput2, sig.ReadData2, sig.immediate, sig.ALUSrc)
    u_Mux2_2 = Mux2(sig.WriteData, sig.ALUResult, sig.MemReadData, sig.MemtoReg)
    u_Adder = Adder(sig.PC4, sig.PC, sig.Const4)
    u_AddSum = Adder(sig.BranchTarget, sig.PC, sig.immediate)
